...
//_ 3EME ETAPE
       ...
       import fs from "fs"
        ...
        //! mise en place de la partie admin pour les pages category


        //! Ici on récupere toutes les category et leurs "datas"
        export const selectAllCategory = async (req, res) => {
        ...
        //! Ici on récupere chaque cathégory et on sépare l'image du reste des données car la manipulation n'est pas la meme

        export  const selectOneCategory = async (req,res) => {
            ...
            try {
                const query = ` 
                //, recupération de toutes les colonnes de la table tea pour ensuite travailler sur chacune d'elle
                SELECT  SELECT category.id as categoryId, title, description, url_img AS urlImg, alt_img
                FROM  tea`;
    ...
    export const createTea = async(req,res) => {
        ...
        const uploadDir = "./public/img/category";
